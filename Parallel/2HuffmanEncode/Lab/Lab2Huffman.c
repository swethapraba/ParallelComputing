...
int main()
{
    ...
    //for the first line new line thing
    int isFirst = 0;
    ...
    for(int x = 0; x < 1; x++) //should be 27 max
    {
        //sprintf -----..............
        fin = fopen( "a01.txt" , "r" );
        //fin = fopen("decodeME.txt", "r"); //temporary to use before doing the other files
        ...
        while ((read = getline(&line, &len, fin)) != EOF) 
        {
            ...
            else if(linesRead == 1) //if you've read the num characters but have to do the spaces
            {
            }
            else if(linesRead == 2) //if you've read in that there will be a space and need the character formula thing
            {
            }
            else if(linesRead > 2 && linesRead < numLines+1)//normal for all the other chars
            {
            }
